...
static const u16 sSoundMovesTable[] =
{
    ...
    MOVE_UPROAR, MOVE_METAL_SOUND, MOVE_GRASS_WHISTLE, MOVE_HYPER_VOICE, MOVE_SNARL, SOUND_MOVES_END
};
    ...
    while (gBattleStruct->turnEffectsBattlerId < gBattlersCount && gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
    {
        ...
        else
        {
            ...
            case ENDTURN_CONVERTED:
                if ((gBattleMons[gActiveBattler].status1 & STATUS1_CONVERTED) && gBattleMons[gActiveBattler].hp != 0)
                {
    ...
    case 0:
        while (gBattleStruct->wishPerishSongBattlerId < gBattlersCount)
        {
             ...
             && gBattleMons[gActiveBattler].hp != 0)
            {
                ...
                if (gWishFutureKnock.futureSightMove[gActiveBattler] == MOVE_DOOM_DESIRE)
                    gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_DOOM_DESIRE;
                if (gWishFutureKnock.futureSightMove[gActiveBattler] == MOVE_PROPHECY)
...
static bool32 IsMonEventLegal(u8 battlerId)
{
    if (GetBattlerSide(battlerId) == B_SIDE_OPPONENT)
        return TRUE;
    if (GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]], MON_DATA_SPECIES, NULL) != SPECIES_GESOMON
        && GetMonData(&gPlayerParty[gBattlerPartyIndexes[battlerId]], MON_DATA_SPECIES, NULL) != SPECIES_FALCOMON)
            return TRUE;
...
u8 IsMonDisobedient(void)
{
    ...
    if (IsMonEventLegal(gBattlerAttacker)) // only false if illegal Mew or Gesomon
    {
