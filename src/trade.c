...
static bool8 BufferTradeParties(void)
{
    ...
    case 21:
        for (i = 0, mon = gEnemyParty; i < PARTY_SIZE; mon++, i++)
        {
            ...
            {
                if (species == SPECIES_BALUCHIMON && GetMonData(mon, MON_DATA_LANGUAGE) != LANGUAGE_JAPANESE)
                {
                    GetMonData(mon, MON_DATA_NICKNAME, name);

                    if (!StringCompareWithoutExtCtrlCodes(name, sJPText_Shedinja))
                    {
                        SetMonData(mon, MON_DATA_NICKNAME, gSpeciesNames[SPECIES_BALUCHIMON]);
                    }
                }
            }
        }
...
static u8 CheckValidityOfTradeMons(u8 *aliveMons, u8 playerPartyCount, u8 playerMonIdx, u8 partnerMonIdx)
{
    ...
    if (partnerSpecies == SPECIES_DEOXYS || partnerSpecies == SPECIES_FALCOMON)
    {
...
static u32 CanTradeSelectedMon(struct Pokemon *playerParty, int partyCount, int monIdx)
{
    ...
    if (species[monIdx] == SPECIES_DEOXYS || species[monIdx] == SPECIES_FALCOMON)
    {
...
{
    if (species == SPECIES_DEOXYS || species == SPECIES_FALCOMON)
    {
        if (!isEventLegal)
            return TRUE;
    }
