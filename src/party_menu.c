...
static void DisplayPartyPokemonGender(u8 gender, u16 species, u8 *nickname, struct PartyMenuBox *menuBox)
{
    ...
    if (species == SPECIES_NONE)
        return;
    if ((species == SPECIES_PUSUMON || species == SPECIES_PUNIMON) && StringCompare(nickname, gSpeciesNames[species]) == 0)
        return;
    switch (gender)
    {
    case MON_MALE:
...
{
    bool32 handleGesomon = TRUE;
    u16 species2;

    // If in a multi battle, show partners Gesomon icon as Normal forme
    if (IsMultiBattle() == TRUE && gMain.inBattle)
        handleGesomon = (sMultiBattlePartnersPartyMask[slot] ^ handleGesomon) ? TRUE : FALSE;

    species2 = GetMonData(mon, MON_DATA_SPECIES2);
    CreatePartyMonIconSpriteParameterized(species2, GetMonData(mon, MON_DATA_PERSONALITY), menuBox, 1, handleGesomon);
    UpdatePartyMonHPBar(menuBox->monSpriteId, mon);
}

static void CreatePartyMonIconSpriteParameterized(u16 species, u32 pid, struct PartyMenuBox *menuBox, u8 priority, bool32 handleGesomon)
{
    if (species != SPECIES_NONE)
    {
        menuBox->monSpriteId = CreateMonIcon(species, SpriteCB_MonIcon, menuBox->spriteCoords[0], menuBox->spriteCoords[1], 4, pid, handleGesomon);
...
void ItemUseCB_IncreaseIV(u8 taskId, TaskFunc task)
{
    ...
    case STAT_HP:
        ...
        {
            modifier = (health <= (MAX_PER_STAT_IVS - 1)) ? (health + 1) : MAX_PER_STAT_IVS;
            SetMonData(mon, MON_DATA_HP_IV, &modifier);
            StringCopy(gStringVar2, gText_HP3);
            didActivate = TRUE;
        }
    ...
    case STAT_ATK:
        ...
        {
            modifier = (attack <= (MAX_PER_STAT_IVS - 1)) ? (attack + 1) : MAX_PER_STAT_IVS;
            SetMonData(mon, MON_DATA_ATK_IV, &modifier);
            StringCopy(gStringVar2, gText_Attack3);
            didActivate = TRUE;
        }
    ...
    case STAT_DEF:
        ...
        {
            modifier = (defense <= (MAX_PER_STAT_IVS - 1)) ? (defense + 1) : MAX_PER_STAT_IVS;
            SetMonData(mon, MON_DATA_DEF_IV, &modifier);
            StringCopy(gStringVar2, gText_Defense3);
            didActivate = TRUE;
        }
    ...
    case STAT_SPEED:
        ...
        {
            modifier = (speed <= (MAX_PER_STAT_IVS - 1)) ? (speed + 1) : MAX_PER_STAT_IVS;
            SetMonData(mon, MON_DATA_SPEED_IV, &modifier);
            StringCopy(gStringVar2, gText_Speed2);
            didActivate = TRUE;
        }
    ...
    case STAT_SPATK:
        ...
        {
            modifier = (spAttack <= (MAX_PER_STAT_IVS - 1)) ? (spAttack + 1) : MAX_PER_STAT_IVS;
            SetMonData(mon, MON_DATA_SPATK_IV, &modifier);
            StringCopy(gStringVar2, gText_SpAtk3);
            didActivate = TRUE;
        }
    ...
    case STAT_SPDEF:
        ...
        {
            modifier = (spDefense <= (MAX_PER_STAT_IVS - 1)) ? (spDefense + 1) : MAX_PER_STAT_IVS;
            SetMonData(mon, MON_DATA_SPDEF_IV, &modifier);
            StringCopy(gStringVar2, gText_SpDef3);
            didActivate = TRUE;
        }
