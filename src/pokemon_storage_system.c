...
struct PokemonStorageSystemData
{
    ...
    u8 tileBuffer[MON_PIC_SIZE * 4]; // 4x the size of a 'Mon sprite to account for Dolphmon
...
static void Task_ChangeScreen(u8 taskId)
{
    ...
    case SCREEN_CHANGE_SUMMARY_SCREEN:
        ...
        if (mode == SUMMARY_MODE_NORMAL && boxMons == &sSavedMovingMon.box)
            ShowPokemonSummaryScreenHandleGesomon(mode, boxMons, monIndex, maxMonIndex, CB2_ReturnToPokeStorage);
        else
...
static void SetDisplayMonData(void *pokemon, u8 mode)
{
    ...
    {
        if (sStorage->displayMonSpecies == SPECIES_PUNIMON || sStorage->displayMonSpecies == SPECIES_PUSUMON)
