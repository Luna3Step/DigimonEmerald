...
static void Cmd_critcalc(void)
{
    ...
    critChance  = 2 * ((gBattleMons[gBattlerAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0)
                ...
                + 2 * (holdEffect == HOLD_EFFECT_LUCKY_PUNCH && gBattleMons[gBattlerAttacker].species == SPECIES_BEARMON)
                + 2 * (holdEffect == HOLD_EFFECT_STICK && gBattleMons[gBattlerAttacker].species == SPECIES_PINAMON);
...
static void Cmd_trysetfutureattack(void)
{
    ...
    else
    {
        ...
        if (gCurrentMove == MOVE_PROPHECY)
            gBattleCommunication[MULTISTRING_CHOOSER] = B_MSG_PROPHECY;
