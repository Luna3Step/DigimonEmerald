...
static const union AnimCmd sSpriteAnim_TypeVaccne[] = {
    ANIMCMD_FRAME(TYPE_VACCNE * 8, 0, FALSE, FALSE),
    ANIMCMD_END
};
...
void ShowPokemonSummaryScreenHandleGesomon(u8 mode, struct BoxPokemon *mons, u8 monIndex, u8 maxMonIndex, void (*callback)(void))
{
    ShowPokemonSummaryScreen(mode, mons, monIndex, maxMonIndex, callback);
    sMonSummaryScreen->handleGesomon = TRUE;
}
...
{
    if (species != SPECIES_PUSUMON && species != SPECIES_PUNIMON)
    {
...
static void SetMonTypeIcons(void)
{
    ...
    else
    {
        ...
        if (gSpeciesInfo[summary->species].type1 != gSpeciesInfo[summary->species].type2)
        {
            ...
            SetTypeSpritePosAndPal(gSpeciesInfo[summary->species].type3, 200, 48, SPRITE_ARR_ID_TYPE + 2);
        }
        else
        {
            SetSpriteInvisibility(SPRITE_ARR_ID_TYPE + 1, TRUE);
...
static u8 LoadMonGfxAndSprite(struct Pokemon *mon, s16 *state)
{
    ...
    case 0:
        if (gMain.inBattle)
        {
            if (ShouldIgnoreGesomonForm(3, sMonSummaryScreen->curMonIndex))
                HandleLoadSpecialPokePic_DontHandleGesomon(&gMonFrontPicTable[summary->species2],
