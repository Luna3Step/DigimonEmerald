...
static void DuplicateGesomonTiles(void *pointer, s32 species);
...
void DecompressPicFromTable(const struct CompressedSpriteSheet *src, void *buffer, s32 species)
{
    if (species > NUM_SPECIES)
        ...
        LZ77UnCompWram(src->data, buffer);
    DuplicateGesomonTiles(buffer, species);
}
...
void LoadSpecialPokePic(const struct CompressedSpriteSheet *src, void *dest, s32 species, u32 personality, bool8 isFrontPic)
{
    ...
    DuplicateGesomonTiles(dest, species);
...
void DecompressPicFromTable_DontHandleGesomon(const struct CompressedSpriteSheet *src, void *buffer, s32 species)
{
    if (species > NUM_SPECIES)
        LZ77UnCompWram(gMonFrontPicTable[0].data, buffer);
    else
...
void HandleLoadSpecialPokePic_DontHandleGesomon(const struct CompressedSpriteSheet *src, void *dest, s32 species, u32 personality)
{
    bool8 isFrontPic;
    ...
    LoadSpecialPokePic_DontHandleGesomon(src, dest, species, personality, isFrontPic);
}

void LoadSpecialPokePic_DontHandleGesomon(const struct CompressedSpriteSheet *src, void *dest, s32 species, u32 personality, bool8 isFrontPic)
{
    if (species == SPECIES_LOPMON)
    {
        u16 i = GET_UNOWN_LETTER(personality);
